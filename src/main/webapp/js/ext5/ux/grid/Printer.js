    ...
    statics: {
        ...
        print: function(grid,html_flg) {
            ...
            if (this.stylesheetPath === null) {
				...
				//Ext.util.CSS.swapStyleSheet('print_css',this.stylesheetPath);
            }
			...
			if(this.title_html)
				var titles='<td colspan="' + columns.length + '">'+this.title_html+'</td>';
			else
				var titles='';
            var pluginsBody = '',
                pluginsBodyMarkup = [];
			...
			if(!_print_style){
				var url=this.stylesheetPath; console.log(url);
				Ext.Ajax.request({
					url: url,async:false,
					success: function(response){
						_print_style = response.responseText;
					}
				});
			}
            ...
            var htmlMarkup = [
                ...
                '<html class="' + Ext.baseCSSPrefix + 'ux-grid-printer">',
                  '<head>',
					...
					'<style type="text/css">'+_print_style+'</style>',
