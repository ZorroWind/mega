...
//    	this.graph.fromJSON(arguments[0]);
//    },
    ...
    initializeEditor: function() {
        ...
        this.initializePaper(arguments[0]);
    ...
    createInspector: function(cellView) {
        ...
        //添加方法 销毁ext combo
        _APP.viewport.down('tpgraphctr')._destroy_combo();
...
//            $('.stencil > .content').append(this.inspector.el);
            $('.inspector-container').html(this.inspector.el);
//            $('.stencil > .content').append(document.getElementsByClassName('inspector-container')[0].childNodes);
        ...
        //cellView改变时 在select对应的位置添加combo
        _APP.viewport.down('tpgraphctr').initStcdCombo(cell);
//        _APP.viewport.down('tpgraphctr').initModelEdit(cell);
    },
    ...
    initializeHaloAndInspector: function() {
        ...
        this.paper.on('cell:pointerup', function(cellView, evt) {
            ...
            //增加自定义事件
            //halo.addHandle({ name: 'myaction', position: 's', icon: 'resources/images/newBtn/addhover-ico.png' });
