...
(function() {
  ...
  callWithJQuery(function($) {
    ...
    pivotTableRenderer = function(pivotData, opts) {
	  var hasIdx,aggregator, c, colAttrs, colKey, colKeys, defaults, i, j, r, result, rowAttrs, rowKey, rowKeys, spanSize, td, th, totalAggregator, tr, txt, val, x;
	  var _delCell = [];
      defaults = {
        localeStrings: {
          totals: "Totals"
      ...
      if (rowAttrs.length !== 0) {
        ...
        for (i in rowAttrs) {
          ...
          tr.appendChild(th);
		  var merge = r.split('-');
		  if(merge.length>1 && merge[0]=="merge") _delCell.push(merge[1]);
		  if(r=='idx') {
			  hasIdx = true;
			  th.hidden = true;
			}
      ...
      for (i in rowKeys) {
        ...
        for (j in rowKey) {
          ...
          if (x !== -1) {
			...
			th.setAttribute("col-index",j);
			th.setAttribute("row-index",i);
			if(hasIdx)th.setAttribute("_idx",rowKey[rowAttrs.length - 1]);
            if (parseInt(j) === rowAttrs.length - 1 && colAttrs.length !== 0) {
              th.setAttribute("colspan", 2);
            }
            tr.appendChild(th);
			if(hasIdx && parseInt(j) === rowAttrs.length - 1) {
				th.hidden = true;
			}
          }
        }
		if(flag==rowAttrs.length)	tr.hidden = true;
        for (j in colKeys) {
		  ...
		  td.setAttribute("col-index",j);
			td.setAttribute("row-index",i);
          tr.appendChild(td);
        }
      ...
      result.appendChild(tr);
	  // 合并单元格
	  if(_delCell.length>0){
		for(var i=_delCell.length-1;i>=0;i--){
			result.rows[0].deleteCell(_delCell[i]);
		  }
		var cell = _delCell[0]-1;
		result.rows[0].cells[cell].colSpan=_delCell.length+1;
		result.rows[0].cells[cell].className = "pvtAxisLabel pvtMerge";
	  }
